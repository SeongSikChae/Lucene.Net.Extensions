...
/// <summary>
/// Phase A: Memory-mapped reads that avoid per-byte ViewAccessor access.
/// Fills BufferedIndexInput via unsafe pointer copy with dispose/read fencing.
/// </summary>
[SupportedOSPlatform("windows")]
public sealed class WinMMapDirectory : WinFSDirectoryBase
{
	/// <summary>Default <see cref="BufferedIndexInput"/> buffer size for mmap reads.</summary>
	public const int DefaultBufferSize = 8 * 1024;

	/// <summary>Creates a memory-mapped directory at <paramref name="path"/>.</summary>
	public WinMMapDirectory(DirectoryInfo path) : base(path, null) { }

	/// <summary>Creates a memory-mapped directory at <paramref name="path"/> with the given lock factory.</summary>
	public WinMMapDirectory(DirectoryInfo path, LockFactory? lockFactory) : base(path, lockFactory) { }

	/// <summary>Creates a memory-mapped directory at <paramref name="path"/>.</summary>
	public WinMMapDirectory(string path) : this(new DirectoryInfo(path)) { }
	...
	public override IndexInput OpenInput(string name, IOContext context)
	{
		...
		return WinMMapIndexInput.Open(fullPath, context);
	}
	...
	internal static IndexInputSlicer CreateSlicerCore(string fullPath, IOContext context)
	{
		...
		try
		{
			return new WinMMapSlicer(owner, context);
		}
		catch
		{
			owner.Dispose();
	...
	internal sealed class WinMMapSlicer : LuceneDirectory.IndexInputSlicer
	{
		private readonly MappingOwner _owner;
		private readonly IOContext _context;
		private int _disposed;

		public WinMMapSlicer(MappingOwner owner, IOContext context)
		{
			_owner = owner;
			_context = context;
		}
		...
		public override IndexInput OpenSlice(string sliceDescription, long offset, long length)
		{
			...
			try
			{
				...
				return new WinMMapIndexInput(
					$"WinMMapIndexInput({sliceDescription} slice={offset}:{offset + length})",
					_owner,
					offset,
					length,
					bufferSize);
	...
	internal sealed class WinMMapIndexInput : BufferedIndexInput
	{
		private readonly MappingOwner _owner;
		...
		public static WinMMapIndexInput Open(string path, IOContext context)
		{
		...
		public WinMMapIndexInput(string resourceDesc, MappingOwner owner, long off, long length, int bufferSize)
		...
		public override object Clone()
		{
			...
			try
			{
				WinMMapIndexInput clone = (WinMMapIndexInput)base.Clone();
				clone._disposed = 0;
				return clone;
			}
			catch
