...
/// <summary>
/// Linux / macOS memory-mapped reads (unified name for RID-based runtime selection).
/// </summary>
	...
	internal sealed unsafe class MappingOwner : IDisposable
	{
		...
		public static MappingOwner Open(string path, long prefetchBytes)
		{
			...
			try
			{
				...
				// Empty files cannot be mapped with a non-zero length view; keep an unmapped owner.
				if (length == 0)
				{
				...
				if (acquired && prefetchBytes > 0)
					UnixMemory.TryPrefetch(ptr, Math.Min(prefetchBytes, length));
		...
		public void TryPrefetchRange(long offset, long length)
		{
			using (_gate.EnterRead())
			{
				...
				UnixMemory.TryPrefetch(_pointer + offset, Math.Min(length, available));
			}
		}
