...
/// <summary>
/// Compares read/search throughput of <see cref="WinRandomAccessDirectory"/> against
/// Lucene.NET <see cref="SimpleFSDirectory"/> and <see cref="NIOFSDirectory"/>.
/// </summary>
[TestClass]
[SupportedOSPlatform("windows")]
public class WinRandomAccessDirectoryPerformanceTests
{
	...
	private static readonly string[] DirectoryKinds =
		["SimpleFSDirectory", "NIOFSDirectory", "WinRandomAccessDirectory"];
	...
	public void CompareReadAndSearchThroughput()
	{
		...
		{
			Assert.Inconclusive("WinRandomAccessDirectory is Windows-only.");
			return;
		}

		string root = Path.Combine(Path.GetTempPath(), "WinRAPerf_" + Guid.NewGuid().ToString("N"));
		...
		try
		{
			...
			string reportPath = Path.Combine(Path.GetTempPath(), "WinRandomAccessDirectory-performance-report.txt");
			System.IO.File.WriteAllText(reportPath, report);
			Trace.WriteLine(report);
			TestContext.WriteLine(report);
			TestContext.AddResultFile(reportPath);

			DirectoryBenchmarkResult? win = results.Find(r => r.Kind == "WinRandomAccessDirectory");
		...
		using (var writer = new IndexWriter(directory, config))
		{
			for (int i = 0; i < DocumentCount; i++)
			{
				var doc = new Document
				{
					...
					new TextField(
						"body",
						$"document {i} lucene windows random-access performance sample text token-{i % 97} token-{(i * 13) % 193}",
						Field.Store.YES),
				};
	...
	private static LuceneDirectory CreateDirectory(string kind, string indexPath) => kind switch
	{
		...
		"WinRandomAccessDirectory" => new WinRandomAccessDirectory(indexPath),
	...
	private static void TryDeleteDirectory(string path)
	{
		...
		{
			// Best-effort cleanup for locked files after dispose.
		}
	}
	...
	private static string FormatReport(IReadOnlyList<DirectoryBenchmarkResult> results)
	{
		...
		sb.AppendLine("WinRandomAccessDirectory performance comparison");
		...
		sb.AppendLine();
		sb.AppendLine($"{"Directory",-28} {"Sequential(ms)",14} {"Random(ms)",12} {"Search(ms)",12} {"Hits",8}");
		...
		{
			sb.AppendLine($"{r.Kind,-28} {r.SequentialReadMs,14} {r.RandomReadMs,12} {r.SearchMs,12} {r.HitsPerSearch,8}");
		}
		...
		if (baseline is not null)
		{
			...
			{
				sb.AppendLine(
					$"{r.Kind,-28} seq={Ratio(r.SequentialReadMs, baseline.SequentialReadMs)}  " +
