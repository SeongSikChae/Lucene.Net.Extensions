...
/// <summary>
/// Compares read/search throughput of <see cref="WinMMapDirectory"/> against
/// Lucene.NET <see cref="SimpleFSDirectory"/> and <see cref="MMapDirectory"/>.
/// </summary>
[TestClass]
[SupportedOSPlatform("windows")]
public class WinMMapDirectoryPerformanceTests
{
	...
	private static readonly string[] DirectoryKinds = ["SimpleFSDirectory", "MMapDirectory", "WinMMapDirectory"];
	...
	public void CompareReadAndSearchThroughput()
	{
		...
		{
			Assert.Inconclusive("WinMMapDirectory is Windows-only.");
			return;
		}

		string root = Path.Combine(Path.GetTempPath(), "WinMMapPerf_" + Guid.NewGuid().ToString("N"));
		...
		try
		{
			...
			string reportPath = Path.Combine(Path.GetTempPath(), "WinMMapDirectory-performance-report.txt");
			System.IO.File.WriteAllText(reportPath, report);
			Trace.WriteLine(report);
			TestContext.WriteLine(report);
			TestContext.AddResultFile(reportPath);

			DirectoryBenchmarkResult? win = results.Find(r => r.Kind == "WinMMapDirectory");
	...
	private static void BuildIndex(string indexPath)
	{
		...
		for (int i = 0; i < DocumentCount; i++)
		{
			var doc = new Document
			{
				...
				new TextField(
					"body",
					$"document {i} lucene windows mmap performance sample text token-{i % 97} token-{(i * 13) % 193}",
					Field.Store.YES),
			};
	...
	private static LuceneDirectory CreateDirectory(string kind, string indexPath) => kind switch
	{
		...
		"WinMMapDirectory" => new WinMMapDirectory(indexPath),
	...
	private static string FormatReport(IReadOnlyList<DirectoryBenchmarkResult> results)
	{
		...
		sb.AppendLine("WinMMapDirectory performance comparison");
		...
		sb.AppendLine();
		sb.AppendLine($"{"Directory",-20} {"Sequential(ms)",14} {"Random(ms)",12} {"Search(ms)",12} {"Hits",8}");
		...
		{
			sb.AppendLine($"{r.Kind,-20} {r.SequentialReadMs,14} {r.RandomReadMs,12} {r.SearchMs,12} {r.HitsPerSearch,8}");
		}
		...
		if (baseline is not null)
		{
			...
			{
				sb.AppendLine(
					$"{r.Kind,-20} seq={Ratio(r.SequentialReadMs, baseline.SequentialReadMs)}  " +
